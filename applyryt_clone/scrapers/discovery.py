...
def search_jobboards(role: str, location: str, hours_old: int) -> list[dict]:
    ...
    # LinkedIn across all US regions
    log.info(f"  [LinkedIn] '{role}' across {len(US_ANCHORS)} US regions...")
    for anchor in US_ANCHORS:
        all_jobs.extend(_search("linkedin", "linkedin", anchor))
        time.sleep(1)  # avoid rate limits between searches

    # Indeed — run nationwide + key cities
    log.info(f"  [Indeed] '{role}' nationwide...")
    for anchor in US_ANCHORS[:5]:
        all_jobs.extend(_search("indeed", "indeed", anchor))

    # Google — non-fatal if 0
