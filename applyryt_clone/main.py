...
if __name__ == "__main__":
    if "--review" in sys.argv:
        ...
        from h1b_lookup import h1b_explanation, get_h1b_priority
        exempt_count  = h1b_cnt.get('🎓 H1B Exempt (Cap-Free)', 0)
        sponsor_count = h1b_cnt.get('✅ H1B Sponsor', 0)
        norecord_count= h1b_cnt.get('❓ No H1B Record', 0)
        print(f"\n  H1B BREAKDOWN:")
        print(f"    🎓 Cap-Exempt (BEST) {exempt_count:4d} jobs  ← No lottery, any time of year")
        print(f"    ✅ H1B Sponsor       {sponsor_count:4d} jobs  ← Proven filer, subject to April lottery")
        print(f"    ❓ No Record         {norecord_count:4d} jobs  ← We still apply, ask during interview")
        print()
        print(h1b_explanation())
        # Sort: Cap-Exempt first, then Sponsor, then No Record
        jobs.sort(key=lambda j: get_h1b_priority(j.get("company","")))
        print(f"\n{'─'*65}")
